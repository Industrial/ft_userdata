...
class newstrategy53_2(IStrategy):
    """
    ...
    sell_deadfish_bb_width = DecimalParameter(
        0.03, 0.75, default=0.05, space="sell", optimize=True
    )
    sell_deadfish_profit = DecimalParameter(
        -0.15, -0.05, default=-0.08, space="sell", optimize=True
    )
    sell_deadfish_bb_factor = DecimalParameter(
        0.90, 1.20, default=1.0, space="sell", optimize=True
    )
    sell_deadfish_volume_factor = DecimalParameter(
        1, 2.5, default=1.5, space="sell", optimize=True
    )
    ...
    low_offset = DecimalParameter(
        0.985, 0.995, default=0.985, space="buy", optimize=True
    )
    high_offset = DecimalParameter(
        1.005, 1.015, default=1.014, space="sell", optimize=True
    )
    high_offset_2 = DecimalParameter(
        1.010, 1.020, default=1.01, space="sell", optimize=True
    )

    sell_trail_profit_min_1 = DecimalParameter(
        0.1, 0.25, default=0.1, space="sell", decimals=3, optimize=True
    )
    sell_trail_profit_max_1 = DecimalParameter(
        0.3, 0.5, default=0.4, space="sell", decimals=2, optimize=True
    )
    sell_trail_down_1 = DecimalParameter(
        0.04, 0.1, default=0.03, space="sell", decimals=3, optimize=True
    )

    sell_trail_profit_min_2 = DecimalParameter(
        0.04, 0.1, default=0.04, space="sell", decimals=3, optimize=True
    )
    sell_trail_profit_max_2 = DecimalParameter(
        0.08, 0.25, default=0.11, space="sell", decimals=2, optimize=True
    )
    sell_trail_down_2 = DecimalParameter(
        0.04, 0.2, default=0.015, space="sell", decimals=3, optimize=True
    )

    pHSL = DecimalParameter(
        -0.500, -0.040, default=-0.08, decimals=3, space="sell", optimize=True
    )

    pPF_1 = DecimalParameter(
        0.008, 0.020, default=0.016, decimals=3, space="sell", optimize=True
    )
    pSL_1 = DecimalParameter(
        0.008, 0.020, default=0.011, decimals=3, space="sell", optimize=True
    )

    pPF_2 = DecimalParameter(
        0.040, 0.100, default=0.080, decimals=3, space="sell", optimize=True
    )
    pSL_2 = DecimalParameter(
        0.020, 0.070, default=0.040, decimals=3, space="sell", optimize=True
    )
        ...
        **kwargs,
        ...
        if last_candle is not None:
            ...
            if (
                (current_profit > 0)
                & (count_of_buys < 4)
                & (last_candle["close"] > last_candle["hma_50"])
                & (
                    last_candle["close"]
                    > (
            ...
            if (
                (current_profit > 0)
                & (count_of_buys >= 4)
                & (last_candle["close"] > last_candle["hma_50"] * 1.01)
                & (
                    last_candle["close"]
                    > (
                        last_candle[f"ma_sell_{self.base_nb_candles_sell.value}"]
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe["ema_vwap_diff_50"] = (
            dataframe["ema_50"] - dataframe["vwap_lowerband"]
        ) / dataframe["ema_50"]
        ...
        informative["sup_level"] = Series(
            np.where(
                sup_series,
                np.where(
                    informative["close"] < informative["open"],
                    informative["close"],
                    informative["open"],
                ),
                float("NaN"),
        ...
        informative["bb_width"] = (
            informative["bb_upperband2"] - informative["bb_lowerband2"]
        ) / informative["bb_middleband2"]
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        btc_dump = dataframe["btc_close"].rolling(24).max() >= (
            ...
            if (pm_arr[i - 1] == final_ub[i - 1] and mavalue[i] <= final_ub[i])
                ...
                if (pm_arr[i - 1] == final_ub[i - 1] and mavalue[i] > final_ub[i])
                    ...
                    if (pm_arr[i - 1] == final_lb[i - 1] and mavalue[i] >= final_lb[i])
                        ...
                        final_ub[i]
                        if (
                            pm_arr[i - 1] == final_lb[i - 1]
                            and mavalue[i] < final_lb[i]
                        )
                        else 0.00
                    )
                )
