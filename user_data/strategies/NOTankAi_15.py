...
class NOTankAi_15(IStrategy):
    ...
    def feature_engineering_expand_basic(self, dataframe, metadata, **kwargs):
        ...
        dataframe["%-HLC3"] = (
            heikinashi["high"] + heikinashi["low"] + heikinashi["close"]
        ) / 3
        ...
        dataframe["%-body_range_ratio"] = (
            dataframe["%-body_size"] / dataframe["%-range_size"]
        )
        dataframe["%-upper_wick_size"] = dataframe["high"] - dataframe[
            ["open", "close"]
        ].max(axis=1)
        ...
        dataframe["%-200sma_dist"] = get_distance(
            heikinashi["close"], dataframe["%-200sma"]
        )
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        dataframe["min_threshold_mean"] = (
            dataframe["minima_sort_threshold"].expanding().mean()
        )
        dataframe["max_threshold_mean"] = (
            dataframe["maxima_sort_threshold"].expanding().mean()
        )
    ...
    def populate_entry_trend(self, df: DataFrame, metadata: dict) -> DataFrame:
        ...
        df.loc[
            (
                ...
                & (
                    df["rsi"] < 30
                )  # RSI abaixo de 30 (condição adicional para limitar entradas)
            ),
        ...
        df.loc[
            (
                ...
                & (
                    df["rsi"] > 70
                )  # RSI acima de 70 (condição adicional para limitar entradas)
            ),
