...
class E0V1E_55(IStrategy):
    ...
    sell_loss_cci_profit = DecimalParameter(
        -0.15, 0, default=-0.15, decimals=2, space="sell", optimize=cci_opt
    )
    ...
    def custom_exit(
        ...
        **kwargs
        ...
        if (
            ...
            or current_candle["close"] > current_candle["ma240"]
        ):
        ...
        if trade.id in TMP_HOLD1:
            if (
                current_candle["high"] > current_candle["ma120"]
                or current_candle["high"] > current_candle["ma240"]
            ):
                if min_profit <= -0.05:
